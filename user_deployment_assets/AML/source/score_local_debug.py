...
def get_ws():
    ...
    svc_pr_id = os.environ.get("SV_PR_ID")
    svc_pr_password = os.environ.get("SV_PR_PSWD")
...
def get_engine():
    ...
    # as described in the documnetation. Use secure
    ...
    # and Ray cluster as descibed in the documentaion
...
#TODO: check with Giulia do we need this block in script.py ?
#TODO: why there is only class C in the result table (maybe non feasible solutions)
...
def get_params(location):
    ...
    for item in item_result:
        temp_dict = {}
        temp_dict= {'class':item[0], 'item':item[1], 'Inventory_Capacity':int(float(item[2])),
        'Max_Quantity':int(float(item[3])),'Holding_Cost':int(float(item[4])),'Order_Cost':int(float(item[5])),
...
def persist_results(result_df_interval,result_df_parameters):
    
    # Convert the string values to number

    result_df_interval = result_df_interval.rename(columns={"timestamp": "Sim_Time"}, errors="raise")
    result_df_parameters = result_df_parameters.rename(columns={"timestamp": "Sim_Time"}, errors="raise")
    result_df_interval['Sim_Time'] = result_df_interval['Sim_Time'].apply(lambda x: x.strftime('%Y-%m-%d %I:%M:%S'))
    result_df_parameters['Sim_Time'] = result_df_parameters['Sim_Time'].apply(lambda x: x.strftime('%Y-%m-%d %I:%M:%S'))
    result_df_parameters['Q'] = result_df_parameters['Q'].str[0]
    result_df_parameters['R'] = result_df_parameters['R'].str[0]

    engine = get_engine()
    try:
            result_df_interval.astype(str).to_sql('sc_results_table_interval', if_exists='append', schema='dbo', con = engine,index=False)
            print("persist Results successfully to the DB")
    except pyodbc.Error as err: # Only error I wanted passed for the test!
            raise err

    try:
            result_df_parameters.astype(str).to_sql('sc_results_table_2', if_exists='append', schema='dbo', con = engine,index=False)
            print("persist Results successfully to the DB")
    except pyodbc.Error as err: # Only error I wanted passed for the test!
            raise err
...
def run_optimization(demand, sim_duration, invCap, maxQ,orderCost,holdCost,leadTime,backLogCost,maxBackLogged,initInv):
    ...
    ## python 3.10 - Var can only accept one between within and domain. Assign the other one
    ## after you have allocated the variable
    ...
    return inv_list, reorder_list, replenish_list, back_list, q_list,r_list
...
def save_res_interval(results_int_df,param,input_location):
    ...
    no_samples = len(results_int_df['Inventory_Level'])
    ...
    res_temp['Class'] = [param['item'] for i in range(0,len(temp_col_inv))]
...
def init():
    ...
    global sales_data, predictor, start
    ...
    predictor = pickle.load(open('/mnt/batch/tasks/shared/LS_root/mounts/clusters/armoshar2/code/Supply-Chain-Management/Deploy/source/downloads/supply-chain-forecast-model/predictor.pkl', 'rb'))



    if ray.is_initialized() == False:
        print("Ray is not up!! We are starting it up with the correct address")
        #ray.init(address="ray://10.0.141.56:10001") ############################### DO this for deployment (head ip)
        ray.init() # CHANGE THIS IS LOCAL ##########################################
    else:
        ray.shutdown()
...
def optimization(forecasts,ensemble,subset_params,sim_duration,input_location):
    ...
    for param in subset_params:
        ...
        for i in range(0,len(ensemble)):
                    ...
                    items.append(ensemble[i]['item'])
                    item_class.append(ensemble[i]['class'])
                    items_location.append(ensemble[i]['location'])
                    ...
                    # COMMENT THESE LINES IF YOU WANT TO RUN LOCAL
                    # inventory_levels,reorder_levels, replenish_level, backlog_levels, Q,R = run_optimization.remote(demand, sim_duration,param['Inventory_Capacity'],
                    ...
                    inventory_levels,reorder_levels, replenish_level, backlog_levels, Q,R = run_optimization(demand, sim_duration,param['Inventory_Capacity'],
                    param['Max_Quantity'],param['Holding_Cost'],param['Order_Cost'],param['Lead_Time'],param['Backlog_Cost'],param['Max_Backlogged'],param['Initial_Inventory'])
...
#                        'Q':ray.get(Q_results), 'R':ray.get(R_results), 'timestamp':run_time}

        result_dict_parameters_ = {'Item':param['item'],'Class':param['class'], 'Location': input_location, 
                        'Q':Q_results, 'R':R_results, 'timestamp':run_time}
        ...
        result_dict_interval_ = {'Item':param['item'],'Class':param['class'], 'Location': input_location, 
                    'Inventory_Level':inventory_results,
                    'Replenish_Index':replenish_results,
                        'Backlog_Level':backlog_results,
                    'Reorder_Index':reorder_results,'timestamp':run_time}
        
        results_int_df = pd.DataFrame.from_dict(result_dict_interval_)
        # COMMENT THIS LINE IF YOU WANT TO RUN LOCAL
        # results_int_df_v2 = save_res_interval.remote(results_int_df,param,input_location)

        results_int_df_v2 = save_res_interval(results_int_df,param,input_location)
        result_df_interval.append(results_int_df_v2)


    result_df_interval_pd = pd.concat(result_df_interval)
    result_df_parameters_pd = pd.concat(result_df_parameters)
    print(result_df_parameters_pd.Q)
    
    # COMMENT THIS LINE IF YOU WANT TO RUN LOCAL
    # persist_results.remote(result_df_interval_pd,result_df_parameters_pd)
    persist_results(result_df_interval_pd,result_df_parameters_pd)
...
def run(Inputs):
    ...
    input_location= str(orjson.loads(body)['data']['Location'])
    params = get_params(input_location)

    
    items_set = set([item['item'] for item in params])
    ...
    ### insert optimization wrapper here


    #TO RUN LOCALLY UNCOMMENT THIS
    #optimization(forecasts,ensemble,subset_params,sim_duration,input_location)

    #FOR Kubernetes uncomment this
    optimization.remote(forecasts,ensemble,subset_params,sim_duration,input_location)
...
if __name__ =='__main__':
    init()
    input_param_dict ={'Location':'Location_0'}
    data = {"data":input_param_dict}
    body = orjson.dumps(data, option=orjson.OPT_NON_STR_KEYS).decode('utf-8')
    run(body)
