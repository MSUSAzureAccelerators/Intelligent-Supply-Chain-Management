...
def persist_results(result_df_interval,result_df_parameters):
    ...
    try:
            result_df_parameters.astype(str).to_sql('sc_results_table', if_exists='append', schema='dbo', con = engine,index=False)
            print("Persisted static optimization results successfully to the DB")
...
def init():
    ...
    absolute_path = os.path.dirname('UseThisToTestScoreFileNotebook.ipynb')
    ...
    if ray.is_initialized() == False:
        print("Ray is not up!! We are starting it up right meow with the correct address")
        ray.init(address="ray://10.0.237.243:10001") ############################### DO this for deployment (head ip)
...
def optimization(forecasts,ensemble,subset_params,sim_duration,input_location):
    ...
    for param in subset_params:
        ...
        result_dict_parameters_ = {'Item':param['item'],'Class':param['class'], 'Location': input_location, 
                       'Q':ray.get(Q_results), 'R':ray.get(R_results),
                        'timestamp':run_time}
...
def run(Inputs):
    ...
    ##TODO: Arashaga to finish this function
    # test = {"run_id": run_id, "locations": location }
    # json.dump(test)
    # return test 
    print("The Run function is")
